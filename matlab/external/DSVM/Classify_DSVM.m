...
indx=1;
while cond==1
    class_x=svmclassifyfn(svmstruct{indx,1},test);
    ...
    function class_out = svmclassifyfn(model,testdata)
        if isa(model,'ClassificationSVM')
            class_out = model.predict(testdata);
        else
            class_out = svmclassify(model,testdata);
        end
    end
end
