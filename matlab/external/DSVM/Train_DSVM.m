function [svmstruct,level] = Train_DSVM(train_data,train_label,kernel_function)
...
if nargin<3 || isempty(kernel_function)
    kernel_function = 'rbf';
end
...
for j=1:nb_class-1
   mat=M(j,:);
    if (mat(1)<=nb_class && mat(2)<=nb_class)
        ...
        svmstruct{n}=svmtrainfn([Classes{n,1};Classes{n,2}],[train_label{mat(1)};train_label{mat(2)}],'Showplot',false, 'Kernel_Function','rbf');
        n=n+1;
    elseif (mat(1)> nb_class && mat(2)> nb_class)
       ...
       svmstruct{n}=svmtrainfn([Classes{n,1};Classes{n,2}],[-nb_class*ones(f1,1);-(nb_class+1)*ones(k1,1)],'Showplot',false, 'Kernel_Function','rbf');
       ...
       A2=[];
        ...
        Classes{n,1}=train_data{mat(mat<=nb_class)};
         ...
         svmstruct{n}=svmtrainfn([Classes{n,1};Classes{n,2}],[train_label{mat(mat<=nb_class)};-1*cluster_ind*ones(l1,1)],'Showplot',false, 'Kernel_Function','rbf');
         n=n+1;
         C1=[];
         C2=[];
    end
...
svmstruct=svmstruct(end:-1:1);
    ...
    function svm_model = svmtrainfn(train_data,train_class,varargin)
        MaxIter = 5000000;
        if verLessThan('matlab','8.6')
            svm_model = svmtrain(train_data,train_class,...
            ...
            switch kernel_function
                case 'linear'
                    csvm_kernel = {'linear'};
                case 'quadratic'
                    csvm_kernel = {'polynomial','PolynomialOrder',2};
                case 'rbf'
                    csvm_kernel = {'gaussian'};
            end
            svm_model = fitcsvm(train_data,train_class,...
                'IterationLimit',MaxIter,'KernelFunction',csvm_kernel{:});
        end
    end
